...
**基于 SQL 抽象语法树结构指纹的 Web 攻击语义检测与攻防可视化平台**

> Semantic Engine for Web Attack Detection —— 不做正则匹配的 WAF，做一个真正"看得懂 SQL 语义"的 WAF。
...
将结构差分结果渲染为可视化语法树，注入产生的变异子树高亮为红色并带脉冲光晕，配合完整的取证信息（判定依据、指纹对照、证据链、SQL 高亮）。
...
> Windows 的 Git Bash 下 `nohup` 启动的进程会随 shell 退出而终止，
> 建议在独立终端窗口中分别执行上述命令。
